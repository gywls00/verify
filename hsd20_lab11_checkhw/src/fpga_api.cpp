		...
		float* output, int M, int N)
{
	...
	for(int n = 0; n < N ; n += SIZE)
	{
		for(int m = 0; m < M ; m += SIZE)
		{
			...
			// 2) Assign a matrix
			// IMPLEMENT THIS
			for (int i = n; i < n + n_remain; ++i) {
				memcpy(mat+(i-n)*SIZE, large_mat+i*SIZE+m, sizeof(float) * m_remain);
				if (m_remain < SIZE)
					memset(mat + (i-n)*SIZE+m_remain, 0, sizeof(float) * (SIZE-m_remain));
				if(n_remain < SIZE)
					memset(mat + (n_remain*SIZE), 0, sizeof(float) * (SIZE-n_remain)*SIZE);
			}
